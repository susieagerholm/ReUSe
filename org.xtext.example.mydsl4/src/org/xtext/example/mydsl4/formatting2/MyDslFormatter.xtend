...
/* 
	def dispatch void format(Robot robot, extension IFormattableDocument document) {
		// TODO: format HiddenRegions around keywords, attributes, cross references, etc. 
		for (Topology topologies : robot.getTopologies()) {
			topologies.format;
		}
		for (Link links : robot.getLinks()) {
			links.format;
		}
		for (Joint joint : robot.getJoint()) {
			joint.format;
		}
		for (AddToLink addtolink : robot.getAddtolink()) {
			addtolink.format;
		}
	}

	def dispatch void format(Topology topology, extension IFormattableDocument document) {
		// TODO: format HiddenRegions around keywords, attributes, cross references, etc. 
		topology.getJoint.format;
		topology.getChild.format;
	}
	*/
	// TODO: implement for Link, AddToLink, Reuse, AssignNewValue, Inertial, Inertia, Visual, Origin, Mass, Collision, Box, Cylinder, Mesh, Sphere, Texture, Color, Joint, Axis, Limit, Calibration, Dynamics, SafetyController, DotExpression
}
